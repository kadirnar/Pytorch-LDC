...
class CoFusion(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.conv1 = conv1x1(in_ch, 96)
        self.conv2 = conv1x3(96, 32)
        self.conv3 = conv3x1(32, 16)
        self.conv4 = conv1x1(16, out_ch)
        self.relu = nn.ReLU()

        self.norm_layer1 = nn.GroupNorm(4, 96)  # before 64
        self.norm_layer2 = nn.GroupNorm(4, 32)  # before 32
        self.norm_layer3 = nn.GroupNorm(4, 16)

    def forward(self, x):
        # fusecat = torch.cat(x, dim=1)
        attn = self.relu(self.norm_layer1(self.conv1(x)))
        attn = self.relu(self.norm_layer2(self.conv2(attn)))
        attn = self.relu(self.norm_layer3(self.conv3(attn)))
        attn = F.softmax(self.conv4(attn), dim=1)
...
class _DenseLayer(nn.Sequential):
    def __init__(self, input_features, out_features):
        ...
        self.add_module("conv1", conv1x3(input_features, out_features, stride=1)),
        self.add_module("norm1", nn.BatchNorm2d(out_features)),
        self.add_module("relu1", nn.ReLU(inplace=True)),
        self.add_module("conv2", conv3x1(out_features, out_features, stride=1)),
...
class DoubleConvBlock(nn.Module):
    def __init__(self, in_features, mid_features, out_features=None, stride=1, use_act=True):
        ...
        self.conv1 = conv1x1(in_features, mid_features, stride=stride)
        self.bn1 = nn.BatchNorm2d(mid_features)
        self.conv2 = conv3x1(mid_features, out_features, stride=1)
        self.bn2 = nn.BatchNorm2d(out_features)
        self.conv3 = conv1x3(out_features, out_features, stride=1)
...
class LDC(nn.Module):
    ...
    def __init__(self):
        ...
        self.dblock_4 = _DenseBlock(5, 64, 128)  # 128
